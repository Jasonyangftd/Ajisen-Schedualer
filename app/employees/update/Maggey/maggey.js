var maggeyUp = angular.module('maggeyUp', ['firebase']);
var headers = ['WorkShift','Monday','Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday','Sunday'];

maggeyUp.controller('maggeyController', function ($scope, $firebase) {
  $scope.heads = headers;
  var fb = new Firebase('https://ajisen-scheduler.firebaseio.com/maggey');
  var sync = $firebase(fb);
  // $scope.employees = sync.$asObject();
  $scope.maggey = sync.$asObject();

  $scope.maggeyUpdate = function(){
    innerFunc = function(){
...
//       maggey.mondayLunch = $(this).val() === "true";
//     });
//   console.log(maggey)
//   // return eileen;
// }
