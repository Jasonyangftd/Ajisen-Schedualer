var wendyUp = angular.module('wendyUp', ['firebase']);
var headers = ['WorkShift','Monday','Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday','Sunday'];

wendyUp.controller('wendyController', function ($scope, $firebase) {
  $scope.heads = headers;
  var fb = new Firebase('https://ajisen-scheduler.firebaseio.com/wendy');
  var sync = $firebase(fb);
  // $scope.employees = sync.$asObject();
  $scope.wendy = sync.$asObject();

  $scope.wendyUpdate = function(){
    innerFunc = function(){
...
//       wendy.mondayLunch = $(this).val() === "true";
//     });
//   console.log(wendy)
//   // return wendy;
// }
