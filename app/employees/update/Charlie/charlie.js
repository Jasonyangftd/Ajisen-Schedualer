var charlieUp = angular.module('charlieUp', ['firebase']);
var headers = ['WorkShift','Monday','Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday','Sunday'];

charlieUp.controller('charlieController', function ($scope, $firebase) {
  $scope.heads = headers;
  var fb = new Firebase('https://ajisen-scheduler.firebaseio.com/charlie');
  var sync = $firebase(fb);
  // $scope.employees = sync.$asObject();
  $scope.charlie = sync.$asObject();

  $scope.charlieUpdate = function(){
    innerFunc = function(){
...
//       charlie.mondayLunch = $(this).val() === "true";
//     });
//   console.log(eileen)
//   // return eileen;
// }
