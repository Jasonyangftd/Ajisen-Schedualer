var vickyUp = angular.module('vickyUp', ['firebase']);
var headers = ['WorkShift','Monday','Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday','Sunday'];

vickyUp.controller('vickyController', function ($scope, $firebase) {
  $scope.heads = headers;
  var fb = new Firebase('https://ajisen-scheduler.firebaseio.com/vicky');
  var sync = $firebase(fb);
  // $scope.employees = sync.$asObject();
  $scope.vicky = sync.$asObject();

  $scope.vickyUpdate = function(){
    innerFunc = function(){
...
//       vicky.mondayLunch = $(this).val() === "true";
//     });
//   console.log(vicky)
//   // return eileen;
// }
