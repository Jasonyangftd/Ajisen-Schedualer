var jeffUp = angular.module('jeffUp', ['firebase']);
var headers = ['WorkShift','Monday','Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday','Sunday'];

jeffUp.controller('jeffController', function ($scope, $firebase) {
  $scope.heads = headers;
  var fb = new Firebase('https://ajisen-scheduler.firebaseio.com/jeff');
  var sync = $firebase(fb);
  // $scope.employees = sync.$asObject();
  $scope.jeff = sync.$asObject();

  $scope.jeffUpdate = function(){
    innerFunc = function(){
...
//       jeff.mondayLunch = $(this).val() === "true";
//     });
//   console.log(eileen)
//   // return eileen;
// }
