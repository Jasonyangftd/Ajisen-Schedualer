var yaoUp = angular.module('yaoUp', ['firebase']);
var headers = ['WorkShift','Monday','Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday','Sunday'];

yaoUp.controller('yaoController', function ($scope, $firebase) {
  $scope.heads = headers;
  var fb = new Firebase('https://ajisen-scheduler.firebaseio.com/yao');
  var sync = $firebase(fb);
  // $scope.employees = sync.$asObject();
  $scope.yao = sync.$asObject();

  $scope.yaoUpdate = function(){
    innerFunc = function(){
