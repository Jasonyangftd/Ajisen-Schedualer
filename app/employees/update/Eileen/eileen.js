var eileenUp = angular.module('eileenUp', ['firebase']);
var headers = ['WorkShift','Monday','Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday','Sunday'];

eileenUp.controller('eileenController', function ($scope, $firebase) {
  $scope.heads = headers;
  var fb = new Firebase('https://ajisen-scheduler.firebaseio.com/eileen');
  var sync = $firebase(fb);
  // $scope.employees = sync.$asObject();
  $scope.eileen = sync.$asObject();

  $scope.eileenUpdate = function(){
    innerFunc = function(){
...
//       eileen.mondayLunch = $(this).val() === "true";
//     });
//   console.log(eileen)
//   // return eileen;
// }
