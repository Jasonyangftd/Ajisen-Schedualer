var karryUp = angular.module('karryUp', ['firebase']);
var headers = ['WorkShift','Monday','Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday','Sunday'];

karryUp.controller('karryController', function ($scope, $firebase) {
  $scope.heads = headers;
  var fb = new Firebase('https://ajisen-scheduler.firebaseio.com/karry');
  var sync = $firebase(fb);
  // $scope.employees = sync.$asObject();
  $scope.karry = sync.$asObject();

  $scope.karryUpdate = function(){
    innerFunc = function(){
...
//       karry.mondayLunch = $(this).val() === "true";
//     });
//   console.log(eileen)
//   // return eileen;
// }
