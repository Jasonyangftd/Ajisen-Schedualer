var evaUp = angular.module('evaUp', ['firebase']);
var headers = ['WorkShift','Monday','Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday','Sunday'];

evaUp.controller('evaController', function ($scope, $firebase) {
  $scope.heads = headers;
  var fb = new Firebase('https://ajisen-scheduler.firebaseio.com/eva');
  var sync = $firebase(fb);
  // $scope.employees = sync.$asObject();
  $scope.eva = sync.$asObject();

  $scope.evaUpdate = function(){
    innerFunc = function(){
