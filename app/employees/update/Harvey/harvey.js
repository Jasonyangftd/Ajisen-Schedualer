var harveyUp = angular.module('harveyUp', ['firebase']);
var headers = ['WorkShift','Monday','Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday','Sunday'];

harveyUp.controller('harveyController', function ($scope, $firebase) {
  $scope.heads = headers;
  var fb = new Firebase('https://ajisen-scheduler.firebaseio.com/harvey');
  var sync = $firebase(fb);
  // $scope.employees = sync.$asObject();
  $scope.harvey = sync.$asObject();

  $scope.harveyUpdate = function(){
    innerFunc = function(){
...
//       harvey.mondayLunch = $(this).val() === "true";
//     });
//   console.log(eileen)
//   // return eileen;
// }
