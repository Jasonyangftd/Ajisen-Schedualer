var samUp = angular.module('samUp', ['firebase']);
var headers = ['WorkShift','Monday','Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday','Sunday'];

samUp.controller('samController', function ($scope, $firebase) {
  $scope.heads = headers;
  var fb = new Firebase('https://ajisen-scheduler.firebaseio.com/sam');
  var sync = $firebase(fb);
  // $scope.employees = sync.$asObject();
  $scope.sam = sync.$asObject();

  $scope.samUpdate = function(){
    innerFunc = function(){
