var sunnieUp = angular.module('sunnieUp', ['firebase']);
var headers = ['WorkShift','Monday','Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday','Sunday'];

sunnieUp.controller('sunnieController', function ($scope, $firebase) {
  $scope.heads = headers;
  var fb = new Firebase('https://ajisen-scheduler.firebaseio.com/sunnie');
  var sync = $firebase(fb);
  // $scope.employees = sync.$asObject();
  $scope.sunnie = sync.$asObject();

  $scope.sunnieUpdate = function(){
    innerFunc = function(){
