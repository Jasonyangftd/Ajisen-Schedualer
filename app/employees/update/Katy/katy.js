var katyUp = angular.module('katyUp', ['firebase']);
var headers = ['WorkShift','Monday','Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday','Sunday'];

katyUp.controller('katyController', function ($scope, $firebase) {
  $scope.heads = headers;
  var fb = new Firebase('https://ajisen-scheduler.firebaseio.com/katy');
  var sync = $firebase(fb);
  // $scope.employees = sync.$asObject();
  $scope.katy = sync.$asObject();

  $scope.katyUpdate = function(){
    innerFunc = function(){
...
//       katy.mondayLunch = $(this).val() === "true";
//     });
//   console.log(katy)
//   // return eileen;
// }
